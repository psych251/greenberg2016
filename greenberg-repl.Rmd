...
d_pilotB <- d_pilotB[d_pilotB$attention_check=='Somewhat disagree',]
...
fit <- lm(postchoice_spread ~ prop_focal_opts_chosen + prechoice_spread, data=d_explicit)
summary(fit)
fit <- lm(postchoice_spread ~ prop_focal_opts_chosen + prechoice_spread, data=d_implicit)
summary(fit)
```
...
```{r}
fit <- lm(focal_opts_eval_post ~ prop_focal_opts_chosen + focal_opts_eval_pre, data=d_explicit)
summary(fit)
fit <- lm(focal_opts_eval_post ~ prop_focal_opts_chosen + focal_opts_eval_pre, data=d_implicit)
summary(fit)
```

We get that prop_focal_opts_chosen does not predict focal_opts_eval_post in the explicit condition but it does in the implicit condition where as the paper got that prop_focal_opts_chosen predicts focal_opts_eval_post in both conditions. However, in the paper they do find that the p value for prop_focal_opts_chosen is an order of magnitude lower in the implicit condition.
...
```{r}
fit <- lm(opp_cost_eval_post ~ prop_focal_opts_chosen + opp_cost_eval_pre, data=d_explicit)
summary(fit)
fit <- lm(opp_cost_eval_post ~ prop_focal_opts_chosen + opp_cost_eval_pre, data=d_implicit)
summary(fit)
```
